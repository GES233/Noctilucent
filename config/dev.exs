...
config :noctilucent, Noctilucent.Repo,
  database: Path.expand("../noctilucent_dev.db", __DIR__),
  pool_size: 5,
  stacktrace: true,
...
config :noctilucent_web, NoctilucentWeb.Endpoint,
  ...
  code_reloader: true,
  debug_errors: true,
  secret_key_base: "eLQ672cm3LKRnA8ZkRCbUxxE/zozN2WbShjTnxcBstIXVh0iROTYfUzqJHQJnHf6",
  watchers: [
    esbuild: {Esbuild, :install_and_run, [:noctilucent_web, ~w(--sourcemap=inline --watch)]},
    tailwind: {Tailwind, :install_and_run, [:noctilucent_web, ~w(--watch)]}
  ]
...
config :noctilucent_web, NoctilucentWeb.Endpoint,
  live_reload: [
    patterns: [
      ...
      ~r"priv/gettext/.*(po)$",
      ~r"lib/noctilucent_web/(controllers|live|components)/.*(ex|heex)$"
    ]
  ]

# 启用开发路由下的控制板和邮箱（邮箱我没用）
config :noctilucent_web, dev_routes: true
